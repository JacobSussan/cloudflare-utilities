...
auth_email="email@example.com"                      # The email used to login 'https://dash.cloudflare.com'
auth_method="global"                                # Set to "global" for Global API Key or "token" for Scoped API Token
auth_key="123abc"                                   # Your API Token or Global API Key
zone_identifier="123abc"                            # Can be found in the "Overview" tab of your domain
record_name="website.com"                           # Which record you want to be synced
record_name_2="*.website.com"                       # Which record you want to be synced
record_name_3="www.website.com"                     # Which record you want to be synced
ttl="3600"                                          # Set the DNS TTL (seconds)
proxy="false"                                       # Set the proxy to true or false
sitename="Example Site"                             # Title of site "Example Site"
...
update_2=$(curl -s -X PATCH "https://api.cloudflare.com/client/v4/zones/$zone_identifier/dns_records/$record_identifier" \
                     -H "X-Auth-Email: $auth_email" \
                     -H "$auth_header $auth_key" \
                     -H "Content-Type: application/json" \
                     --data "{\"type\":\"A\",\"name\":\"$record_name_2\",\"content\":\"$ip\",\"ttl\":$ttl,\"proxied\":${proxy}}")

update_3=$(curl -s -X PATCH "https://api.cloudflare.com/client/v4/zones/$zone_identifier/dns_records/$record_identifier" \
                     -H "X-Auth-Email: $auth_email" \
                     -H "$auth_header $auth_key" \
                     -H "Content-Type: application/json" \
                     --data "{\"type\":\"A\",\"name\":\"$record_name_3\",\"content\":\"$ip\",\"ttl\":$ttl,\"proxied\":${proxy}}")
